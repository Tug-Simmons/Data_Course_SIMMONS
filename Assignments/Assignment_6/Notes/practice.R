...
cv <- read_csv("./Exams/Exam_1/cleaned_covid_data.csv")
cv <- data.frame(cv)
...
A_states <- cv[grep("^A", cv$Province_State), ]
...
cv$Last_Update <- as.Date(cv$Last_Update)

ggplot(A_states, aes(x = Last_Update,
                     y = Deaths)) +
  geom_point() +
  stat_smooth(method = "loess", formula = y ~ x, se = FALSE, col = "red") +
  ggtitle("Deaths From Covid Over Time") +
...
state_max_fatality_rate <- cv %>% 
  group_by(Province_State) %>%
  summarize(Maximum_Fatality_Ratio = max(Case_Fatality_Ratio, na.rm = TRUE)) %>%
  arrange(desc(Maximum_Fatality_Ratio))

state_max_fatality_rate <- data.frame(state_max_fatality_rate)
...
state_max_fatality_rate$Province_State <- factor(state_max_fatality_rate$Province_State, 
                                                 levels = state_max_fatality_rate$Province_State[order(state_max_fatality_rate$Maximum_Fatality_Ratio, decreasing = TRUE)])
